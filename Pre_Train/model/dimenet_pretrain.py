...
class DimeNetPlusPlus(torch.nn.Module):
    ...
    def __init__(
        ...
        self.attn = nn.Linear(2*hidden_channels, hidden_channels)
    ...
    def triplets(self, edge_index, x_in, num_nodes):
        ...
        return col, row, idx_i, idx_j, idx_k, idx_kj, idx_ji, x_in#out_nodes_features
    

    def triplets_inter(self, edge_index, x_in, num_nodes):
        row, col = edge_index  # j->i
        #x_in[N, FIN], dense_adj_t [N, N]
        # row, col = col, row  # Swap because my definition of edge_index is i->j
        value = torch.arange(row.size(0), device=row.device)
    ...
    def triplets_intra(self, edge_index, x_in, num_nodes):
        row, col = edge_index  # j->i
        #x_in[N, FIN], dense_adj_t [N, N]
        # row, col = col, row  # Swap because my definition of edge_index is i->j
        value = torch.arange(row.size(0), device=row.device)
...
class DimeNetPlusPlusWrap(DimeNetPlusPlus):
    ...
    def forward(self, data):
        ...
        _, _, idx_i, idx_j, idx_k, idx_kj, idx_ji, x_att__ = self.triplets(
                edge_index, x_att, num_nodes=data.atom_types.size(0)
        )
        x_att_1 = self.triplets_intra(edge_index[:,(1-edge_attr[:,1]).bool()],x_att, num_nodes=data.atom_types.size(0))

        x_att_2 = self.triplets_inter(edge_index[:,edge_attr[:,1].bool()],x_att, num_nodes=data.atom_types.size(0))
        x_att = self.attn(torch.cat([x_att_1,x_att_2],-1))

        x= self.emb_con(x_att, rbf, i, j)
        # Calculate angles.
        idx_i = idx_i.cuda()
        idx_j =idx_j.cuda()
        idx_k=idx_k.cuda()
        ...
        return P, energy, gt_edge.long(), edge_attr_whole
